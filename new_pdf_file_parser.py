...
def parse_pdf_new(doc, full_text: str) -> Program:
    try:
        ...
        # --- 3. Rodzaj materiału – pobieramy z linii po "Program glówny:"
        prog_glowny_match = re.search(r"Program glówny\s*\n\s*(.+)", full_text)
        if prog_glowny_match:
            prog_line = prog_glowny_match.group(1).strip()
            material_match = re.search(r"\((\d+\.\d+)\)", prog_line)
            if material_match:
        ...
        # --- 4. Czas trwania maszyny – usuwamy część w nawiasach kwadratowych
        ...
        # Zwracamy domyślny obiekt Program, aby nie zwracać None
